...
public class Simulator extends Application {
	...
	public void start(Stage primaryStage) throws Exception {
		...
		operationButtons.getChildren().addAll(knownVariables, separator1, read, write, swap, init, message, separator2,
				interpret);
		// Add operation buttons
		...
		controlButtons.getChildren().addAll(transmit, transmitAll, continuousTransmit, separator3, clearLists,
				importList, exportQueued, exportSent, separator4, showMembers);
		// Add lists
	...
	private ToggleButton buildContinousTransmitButton() {
		...
		continuousTransmit.setTooltip(
				new Tooltip("Trasmit all operations automatically. Creates new random operations if neccessary."));
	...
	private Label buildWaitingOperationsLabel() {
		...
		// waitingOperations.setText("#Waiting: " +
		// getNumberWaitingOperations());
	...
	private class StreamSimulator implements CommunicatorListener {
		...
		private HashMap<String, List<String>> buildSources() {
			...
			tmp1.addAll("    public void show (Operation op){", "        if (op == null) {", "            return;",
			...
			tmp2.addAll("    public void trySources (Map<String, List<String>> sources){",
					"        if (sources == null) {", "            return;", "        }",
					"        this.getTabs().clear();", "        nameTabMapping.clear();", "        int tabNumber = 0;",
		...
		public void initOperation() {
			...
			op.setValue(new double[] { Math.round(Math.random() * 100), Math.round(Math.random() * 100),
