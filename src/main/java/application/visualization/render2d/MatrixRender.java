...
public class MatrixRender extends Render {
	 ...
	 * 
	 * @param struct
	 *            The structure to draw as a Matrix.
	 * @param optionNumber
	 *            Indicates whether this MatrixRender is COLUMN or ROW major.
	 * @param width
	 ...
	 */
	public MatrixRender(DataStructure struct, int optionNumber, double width, double height, double hspace,
			double vspace) {
	...
	public void render() {
		if (struct.repaintAll) {
			...
			List<Element> inactiveElements = struct.getInactiveElements();
			for (Element e : struct.getElements()) {
				if (inactiveElements.contains(e)) {
	...
	/**
	 * Draw an element. Style can be fetched using {@code e.getColor()}, or use
	 * {@code null} for the default color.
	 * 
	 * @param e
	 *            The element to draw.
	 * @param style
